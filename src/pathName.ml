...
type t = { path : Name.t list; base : Name.t }
...
let of_name (path : Name.t list) (base : Name.t) : t = { path; base }
...
let of_long_ident (is_value : bool) (long_ident : Longident.t) : t Monad.t =
  ...
  let* () =
    ...
    | head :: _ -> (
        let require =
          Configuration.should_require_long_ident configuration head
        in
        match require with
        | None -> return ()
...
let split_ident (is_value : bool) (ident : Ident.t) :
  ...
  | base :: path ->
      let base =
        match path with [] -> base | _ :: _ -> String.capitalize_ascii base
      in
...
let map_constructor_name (cstr_name : string) (typ_ident : string) :
  ...
  let* configuration = get_configuration in
  let renaming =
    Configuration.is_constructor_renamed configuration typ_ident cstr_name
  in
  match renaming with
...
let to_name (is_value : bool) (x : t) : Name.t Monad.t =
  let s = to_string x |> String.map (fun c -> if c = '.' then '_' else c) in
...
let is_record (path : Path.t) : bool Monad.t =
  let* env = get_env in
  match Env.find_type path env with
...
let prim_proj_fst : t =
  { base = Name.of_string_raw "fst"; path = [ Name.of_string_raw "Primitive" ] }

let prim_proj_snd : t =
  { base = Name.of_string_raw "snd"; path = [ Name.of_string_raw "Primitive" ] }
